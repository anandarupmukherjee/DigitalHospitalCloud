...
class Command(BaseCommand):
    ...
    def _handle_print_message(raw_payload: str) -> None:
        ...
        product_code = (
            ...
            or product_payload.get("product_code")
            or product_payload.get("barcode_value")
        )
        raw_qrcode_value = (
            data.get("qrcode_value")
            or data.get("qrcode")
            or data.get("qrCode")
            or product_payload.get("qrcode_value")
        )

        product_name_candidates = []

        def _maybe_append(value: str):
            if isinstance(value, str):
                value = value.strip()
                if value:
                    product_name_candidates.append(value)

        _maybe_append(data.get("productName"))
        _maybe_append(product_payload.get("name"))
        _maybe_append(data.get("requestedName"))
        ...
        if raw_qrcode_value:
            try:
                numeric_qrcode = int(str(raw_qrcode_value).strip())
            except (TypeError, ValueError):
                logger.warning("Invalid QR code value in print message: %s", raw_qrcode_value)

        product = None
        if numeric_qrcode is not None:
            product = Product.objects.filter(qr_numeric_code=numeric_qrcode).first()
        if not product and product_code:
            product = Product.objects.filter(product_code__iexact=product_code).first()
        if not product and product_name_candidates:
            matcher = ProductMatcher(
                getattr(settings, "DATA_OUTPUT_FUZZY_THRESHOLD", 0.6)
            )
            for candidate in product_name_candidates:
                product, _score = matcher.find(candidate)
                if product:
        ...
        if not product:
            logger.warning(
                "No matching Product found for print message (code=%s, qr=%s, tried_names=%s)",
                product_code,
                raw_qrcode_value,
                product_name_candidates,
            )
